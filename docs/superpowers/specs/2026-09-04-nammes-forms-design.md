...
**`one_response_per_person` is enforced by a `SECURITY DEFINER` trigger, not a table-level unique index or an RLS policy subquery.** A static unique index on `(form_id, respondent_id)` can't see another table's per-row flag, so it would incorrectly block a second submission even on a `require_signin = true` form where the admin left `one_response_per_person` off (a valid combination — e.g. a suggestion box that requires sign-in but allows repeat posts). The first attempt at fixing that moved the check into the `form_responses` insert policy as a `not exists` subquery against `form_responses` itself — which hit a real bug in production: Postgres refuses to evaluate an RLS policy that queries its own table from within itself, raising `infinite recursion detected in policy for relation "form_responses"` on every submit. The fix is a `before insert` trigger calling a `SECURITY DEFINER` function: the function runs with the privileges of its owner (bypassing RLS for its internal lookup), so its `exists (select 1 from form_responses where ...)` check doesn't re-trigger the same policy chain. The insert policy itself now only handles the `require_signin` check.
...
- `form_responses`:
  - `insert` (RLS policy): allowed if the form's `require_signin = false` (any visitor, `respondent_id` left null), or if `require_signin = true` and `auth.uid() = respondent_id`. Enforced via a policy that joins back to `forms` on `form_id` — no self-reference to `form_responses`.
  - `insert` (trigger, separate from RLS): a `before insert` trigger running `enforce_one_response_per_person()` (`SECURITY DEFINER`) raises an exception if the form's `one_response_per_person = true` and a row for that `(form_id, respondent_id)` already exists. Only applies when `respondent_id is not null`.
